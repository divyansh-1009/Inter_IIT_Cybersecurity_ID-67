    ...
    enum ContentType content_type, word16 length)
{
    ...
    /* seq[0] combines the epoch and 16 MSB of sequence number. We write on the
       ...
       number */
    printf("c16toa entering 1st time\n");
    c16toa((word16)(seq[0] >> 16), hdr->epoch);
    printf("c16toa done 2nd time\n");
    c16toa((word16)seq[0], hdr->sequenceNumber);
    printf("c16toa done 3rd time\n");
    c32toa(seq[1], &hdr->sequenceNumber[2]);

    printf("c32toa done 1st time\n");
...
static int Dtls13SendFragment(WOLFSSL* ssl, byte* output, word16 output_size,
    ...
    int sendImmediately)
{
    printf("entering Dtls13SendFragment\n");
    ...
    printf("isProtected done\n");
    recordHeaderLength = Dtls13GetRlHeaderLength(ssl, isProtected);
    printf("recordHeaderLength done\n");

    if (length <= recordHeaderLength){
        printf("length less than recordHeaderLength\n");
        return BUFFER_ERROR;
    }

    printf("no buffer_error\n");

    recordLength = length - recordHeaderLength;

    if (!isProtected) {
        printf("enter Dtls13RlAddPlaintextHeader\n");
        ret = Dtls13RlAddPlaintextHeader(ssl, output, handshake, recordLength);
        printf("Dtls13RlAddPlaintextHeader done\n");
        if (ret != 0){
            printf("ret is not 0\n");
            return ret;
        }
        ...
        if (hashOutput) {
            ...
            if (ret != 0){
                printf("ret is not 0 second time\n");
                return ret;
            }
        }
        ...
        if (sendLength < 0){
            printf("sendLength is less than 0\n");
            return sendLength;
        }
    ...
    printf("before FragIsInOutputBuffer function\n");
    if (!FragIsInOutputBuffer(ssl, output))
    ...
    printf("Dtls13SendFragment completed\n");
    return ret;
}
...
static WC_INLINE void Dtls13FreeRtxBufferRecord(WOLFSSL* ssl,
    Dtls13RtxRecord* r)
{
    printf("Entering Dtls13FreeRtxBufferRecord\n");
    (void)ssl;
    ...
    word16 length, enum HandShakeType handshakeType, w64wrapper seq)
{
    ...
    if (ssl->dtls13EncryptEpoch == NULL){
        printf("Dtls13RtxNewRecord: no encrypt epoch!\n");
        return NULL;
    }
    ...
    printf("Dtls13RtxNewRecord completed\n");
    return r;
}

static void Dtls13RtxAddRecord(Dtls13Rtx* fsm, Dtls13RtxRecord* r)
{
    printf("Entering Dtls13RtxAddRecord\n");
    ...
    word32 length, int hashOutput)
{
    ...
    if (handshakeType != hello_retry_request) {
        ...
        if (rtxRecord == NULL){
            printf("Dtls13SendOneFragmentRtx: memory error\n");
            return MEMORY_E;
        }
    }
    ...
    printf("before rtx != null function\n");
    if (rtxRecord != NULL) {
        printf("rtxRecord is not null\n");
        if (ret == 0 || ret == WC_NO_ERR_TRACE(WANT_WRITE))
    ...
    printf("Dtls13SendOneFragmentRtx completed\n");
    return ret;
}
